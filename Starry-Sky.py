...
async def setactivity(ctx, *, status):
    if ctx.author.id in subowner or ctx.author.id == 584008752005513216:
...
@bot.command(aliases=["chedit","che"],description="指定したチャンネルの名前を変更するよ。\n`[channel]`に今の名前\n`[name]`に新しいチャンネル名\nチャンネルを管理できる人のみ。")
async def channeledit(ctx,channel:discord.TextChannel,name):
    if (ctx.guild.me.top_role < ctx.author.top_role and ctx.author.guild_permissions.manage_channels) or ctx.guild.owner == ctx.author:
        await channel.edit(name=name)
        e = discord.Embed(title="操作成功", description=f'チャンネル名を変更したよ。\n現在のチャンネル名:{name}',color=ctx.author.color,timestamp=datetime.datetime.utcnow())
        await ctx.send(embed=e)
    else:
        await s.no_per(ctx,"channel",0xff0000)
#メッセージ系コード
    ...
    try:
        if (ctx.guild.me.top_role < ctx.author.top_role and ctx.author.guild_permissions.manage_messages) or ctx.guild.owner == ctx.author:
            ...
            try:
                ...
                return
            if guess.content == "ok":
                e = discord.Embed(title="全メッセージ削除",description="チャンネルの全メッセージ削除を開始するよ。")
...
async def info_(ctx):
    ...
    fist.set_thumbnail(url=ctx.bot.user.avatar.url)
    e1 = discord.Embed(title="BOT情報 - Info", color=0x03a9fc)
    e1.set_thumbnail(url=ctx.bot.user.avatar.url)
    ...
    e2.set_thumbnail(url=ctx.bot.user.avatar.url)
    ...
    e3.set_thumbnail(url=ctx.bot.user.avatar.url)
...
async def admins(ctx):
    e=discord.Embed(title="BOT管理者",description=f"{bot.user.name}の管理者達だよ。",color=0x03a9fc)
...
async def roleinfo(ctx, *, role:discord.Role):
    e = discord.Embed(title=f"役職情報 - {role.name}", description="",color=role.color,timestamp=datetime.datetime.utcnow())
    ...
    e.set_footer(icon_url=ctx.author.avatar.url,text=f"実行者:{ctx.author}")
    await ctx.send(embed=e)
...
async def usersearch(ctx, user_id=""):
    ...
    try:user = await bot.fetch_user(int(user_id))
        ...
        embed.set_footer(icon_url=ctx.author.avatar.url,text=f"実行者:{ctx.author}")
...
async def userinfo(ctx, *, user: discord.Member=None):
    ...
    if user == None:
        ...
        if len(ctx.author.roles) <= 1000:embed.add_field(name=f"役職 - {len(ctx.author.roles)}:",value=",".join(c.mention for c in list(reversed(ctx.author.roles))), inline=False)
        else:embed.add_field(name=f"役職 - {len(ctx.author.roles)}:",value='多すぎて表示できないよ。', inline=False)
        ...
        if len(user.roles) <= 1000:embed.add_field(name=f"役職 - {len(ctx.author.roles)}:",value=",".join(c.mention for c in list(reversed(user.roles))), inline=False)
        else:embed.add_field(name=f"役職 - {len(ctx.author.roles)}:",value='多すぎて表示できないよ。', inline=False)
...
@bot.command(description="名前から対象のIDを取得するよ。\n`[type]`には取得する対象名、`[name]`には名前を入れてね。\n~~あんま需要ないかも by開発者~~")
@commands.is_owner()
async def get_id(ctx,type,*,name):
    if type == "user":
        u = discord.utils.get(bot.users, name=name)
        return await ctx.send(u.id)
                        
    elif type == "guild":
        g = discord.utils.get(bot.guilds, name=name)
        return await ctx.send(g.id)
                        
    elif type == "emoji":
        e = discord.utils.get(bot.emojis, name=name)
        return await ctx.send(e.id)

@bot.command(aliases=["joins"],description="Botが導入されているサーバーを表示するよ。")
async def joinserverlist(ctx):
    await ctx.send(embed=discord.Embed(title=f"導入されているサーバー一覧 ({len(ctx.bot.guilds)}サーバー)",description="\n".join([guild.name for guild in bot.guilds]),color=col))                       
##### 一般ユーザー系コマンド #####
...
@bot.command(aliases=["usericon","uico"],description="指定したユーザーのアイコンを表示するよ。\n引数がない場合、実行者のアイコンが表示されるよ。")
async def avatar(ctx,*,user:discord.Member=None):
    if user is None:
        e=discord.Embed(title=f"{ctx.author.name}さんのアイコン",color=ctx.author.color)
        e.set_image(url=ctx.author.avatar.url)
        await ctx.send(embed=e)
    else:
...
@bot.command(description="文字列をシャッフルするよ。")
async def rands(ctx,*,word):
    list_word=list(word);
    sore=""
    for num in range(len(list_word)):
        sore=sore+list_word[random.randrange(len(list_word))]
        if num==len(list_word)-1:
            break
            
    return await ctx.send(sore)
##### メッセージ #####            
...
@bot.command(aliases=["task"],description="メモをDMに送信するよ。")
async def memo(ctx,*,memo):
    await ctx.message.add_reaction("✅")
    e=discord.Embed(title="メモ",description=f"```\n{memo}\n```",timestamp=datetime.datetime.utcnow(),color=col)
...
async def poll(ctx,*content):
    if len(content) == 1:
        ...
        embed.set_footer(icon_url=ctx.author.avatar.url,text=f"作成者:{ctx.author.name}")
        msg = await ctx.send(embed=embed)
        [await msg.add_reaction(emoji) for emoji in emojis]
                        
@bot.command(aliases=["src"],description="Starry Skyのソースコードのリンクを表示するよ。")
async def source(ctx):
    await ctx.send("https://github.com/Mr-ShiRoNo/Starry-Sky-Source")
                        
@bot.command(aliases=["rnum"],description="乱数を生成するよ。\n`snum`に初めの数、`enum`に終わりの数を入れてね。")
async def randomnumber(ctx,snum:int,enum:int):
    n = random.randint(snum,enum)
    await ctx.send(n)
#ログ
@bot.event
async def on_member_join(member):
    print(f'[Join Log] {member}さんが{member.guild}(ID:{member.guild.id})に参加しました。({member.joined_at})')
                        
@bot.event
async def on_member_remove(member):
    print(f'[Leave Log] {member}さんが{member.guild}(ID:{member.guild.id})から退出しました。({datetime.datetime.utcnow()})')
                        
@bot.event
async def on_command(ctx):
    if isinstance(ctx.message.channel, discord.DMChannel):
        e = discord.Embed(title="コマンドログ", color=bcol)
        ...
        await bot.get_channel(848411882002972682).send(embed=e)
    else:
        e = discord.Embed(title="コマンドログ", color=bcol)
...
async def on_command_error(context,exception):
    if isinstance(exception, commands.CommandNotFound):
        ...
        e = discord.Embed(title="コマンドエラー",description=f"{context.author.name}さん、`{context.message.content}`っていうコマンドは無いよ。\n`s!help`で確認してね。", color=0xff0000)
        e.add_field(name="もしかして:",value="`{}`".format(des))
        await context.send(embed=e)
        print (f"[Error Log] コマンド名エラー:『{context.message.content}』を{context.message.author}さんが間違えました。\nサーバー:{context.message.guild}")
    elif isinstance(exception, commands.MissingRequiredArgument):
        await context.send(embed=discord.Embed(title="コマンドエラー - 引数不足",description="コマンドのパラメーターが不足しているよ。",color=0xff0000))
        print (f"[Error Log] コマンドパラメーターエラー:『{context.message.content}』を{context.message.author}さんが間違えました。\nサーバー:{context.message.guild}")
    elif isinstance(exception,commands.NotOwner):
        await context.send(embed=discord.Embed(title="実行エラー - オーナーコマンド",description="あなたはBOTオーナーじゃないよ。",color=0xff0000))
        print (f"[Error Log] オーナー専用コマンド実行:『{context.message.content}』を{context.message.author}さんが実行しました。\nサーバー:{context.message.guild}")
        ...
        embed = discord.Embed(title="権限エラー", description="BOTに権限がないから、コマンドを実行できないよ。", color=0xff0000)
        await context.send(embed=embed)
    elif isinstance(exception, discord.ext.commands.errors.MissingPermissions):
        embed = discord.Embed(title="権限エラー", description="あなたに権限がないから、コマンドを実行できないよ。", color=0xff0000)
        await context.send(embed=embed)
    else:
        e = discord.Embed(title="‼Exception Occurred‼", description=f"例外が発生しました。\n```\n{exception}\n```", color=0xff0000)
        print (f"[Error Log] 例外が発生しました。\n『{exception}』\n発生サーバー:{context.message.guild}\n発生ユーザー:{context.message.author}")
        await context.send(embed=e)

#ヘルプ
@bot.command(aliases=["ho"],description="制作者用コマンドヘルプを表示するよ。\nBOT製作者しか使えないよ。")
@commands.is_owner()
async def help_owner(ctx):
    e = discord.Embed(title="Command Help Owner",description="コマンドの先頭には、必ず`s!`がいるよ。",color=0x03a9fc)
    e.add_field(name="Debug commands:",value="`command`,`remove`,`eval`,`valu`,`reboot`,`down`,`shell`,`jishaku`",inline=False)
    e.add_field(name="Message commands:",value="`send`,`senddm`,`secretsenddm`,`github`",inline=False)
    e.add_field(name="Status&Activity commands:",value="`setactivity`,`resetactivity`,`setonline`,`setidle`,`setdnd`",inline=False)
    e.add_field(name="Other commands:",value="`leaveguild`,`get_id`",inline=False)
...
async def help(ctx,name=None):
    if name is not None:
        ...
        e2 = discord.Embed(title="一般ユーザー向けコマンド",description="Message:`makeembed`,`say`,`messagehistory`,`rands`\nVote:`poll`\nPlay:`randomnumber`,`dice`,`omikuji`",color=0x03a9fc)
        e2.set_footer(text="HelpPage(3/9)")
        e3 = discord.Embed(title="未分類(?)コード",description="`source`,`memo`",color=0x03a9fc)
        e3.set_footer(text="HelpPage(4/9)")
        e4 = discord.Embed(title="情報コマンド",description="`userinfo`,`usersearch`,`serverinfo`,`roleinfo`,`channelinfo`,`boottime`,`avatar`",color=0x03a9fc)
        e4.set_footer(text="HelpPage(5/9)")
        e5 = discord.Embed(title="役職コマンド",description="`rolecreat`,`roledelete`,`roleadd`,`roleremove`,`roleusers`,`roleedit`,`roleallmembersadd`,`roleallmembersremove`",color=0x03a9fc)
        ...
        e8 = discord.Embed(title="管理者専用コマンド",description="`eval`,`setactivity`,`resetactivity`,`setonline`,`setidle`,`setdnd`,`send`,`senddm`",color=0x03a9fc)
